...
import * as videoApi from "@/services/api/video";
...
const task: videoApi.VideoGenerationTask = { id: "video-1", modelConfigId: 7, model: "media-video-test", status: "running", pollAfterMs: 1200 };
...
describe("media video task API", () => {
    it("does not expose server-side cancellation", () => {
        expect(videoApi).not.toHaveProperty("cancelVideoGenerationTask");
    });
    ...
    it("resolves the server model and creates a same-origin domain request", async () => {
        ...
        const created = await videoApi.createVideoGenerationTask(config("media-video-create", "secret-key"), "ocean at dusk");
        ...
        expect(axios.post).toHaveBeenCalledWith(
            "/v1/videos",
            expect.objectContaining({ model_config_id: 7, prompt: "ocean at dusk", reference_images: [], reference_videos: [], reference_audios: [] }),
    ...
    it("maps running and failed server states without provider routing", async () => {
        ...
        await expect(videoApi.pollVideoGenerationTask(config(task.model), task)).resolves.toEqual({ status: "pending", task: { ...task, pollAfterMs: 2000 } });
        await expect(videoApi.pollVideoGenerationTask(config(task.model), task)).resolves.toEqual({ status: "failed", task: { ...task, status: "failed", pollAfterMs: undefined }, error: "upstream rejected" });
    });
    ...
    it("reads completed content when the task has no result URL", async () => {
        ...
        const state = await videoApi.pollVideoGenerationTask(config(task.model), task);
