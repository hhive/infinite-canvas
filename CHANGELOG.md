...
- [修复] 无限画布生成配置节点底部的「开始生成」按钮不再被节点高度裁掉，超出内容改为在面板内滚动，并提高生成配置节点默认高度使新建节点无需滚动。
- [修复] 视频任务轮询遇到临时网络中断时会在后端截止时间内继续查询，不再把仍在后台生成的任务立即标记为失败。
- [优化] 图片、视频、文本与音频工作台的模型选择器支持按显示名、模型名和供应商搜索，并补充键盘选择与无结果提示；模型广场新增关键字、媒体类型和供应商筛选。
- [修复] 图片任务完成后记录轮询、解析与本地保存阶段耗时；保存超过 15 秒只告警、不再人为判定失败，生图工作台同时移除一次重复的大图元数据解码。
- [修复] 生图工作台收到任务完成结果后立即展示图片并结束生成态，本地保存与历史日志转为后台收尾；页面从后台恢复可见时立即查询任务状态。
- [修复] 视频工作台与无限画布视频收到完成结果后立即使用后端 URL 展示并结束生成态，本地视频保存后台执行；页面恢复可见时立即继续视频轮询。
- [修复] Sub2API Cookie 会话在无限画布生图时不再被误判为未配置，并为 Gemini 渠道补充原生模型列表获取接口。
- [合并] 同步主分支插件节点、模型脚本、全站 Agent 与国际化能力，同时保留 Media 同源图片/视频任务、任务恢复和 API Key 分组定制。
- [修复] 视频完成后本地持久化超过 8 秒时使用媒体站自有结果 URL，避免无限画布长期停留在请求中。
- [兼容] 配置了脚本的图片/视频模型使用插件执行器，普通 Media 模型继续使用同源异步任务接口；历史字符串模型配置仍可正常选择。
- [新增] Sub2API Cookie 会话可按图片或视频能力自动或手工切换本人 API Key，并同步刷新两类模型目录。
- [调整] 图片与视频模型按模型名称提交请求，用户界面完整展示显示名称，模型广场同时展示显示名称和模型名称。
